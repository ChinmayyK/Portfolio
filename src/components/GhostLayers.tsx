...
// 5. Network Topology (For ClipRelay)
export function NetworkTopology() {
  ...
  return (
    <motion.div 
      ...
      style={{
        y,
        WebkitMaskImage: "radial-gradient(ellipse 70% 70% at 70% 40%, white 10%, transparent 80%)",
        maskImage: "radial-gradient(ellipse 70% 70% at 70% 40%, white 10%, transparent 80%)"
      }}
    >
      <svg className="absolute w-full h-full opacity-[0.05] dark:opacity-[0.08]" viewBox="0 0 1000 1000" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
        <g stroke="currentColor" fill="none" className="text-[var(--teal)]">
          {/* Base grid dots */}
          <pattern id="net-grid" width="40" height="40" patternUnits="userSpaceOnUse">
            <circle cx="20" cy="20" r="1" fill="currentColor" opacity="0.5" />
          </pattern>
          <rect width="100%" height="100%" fill="url(#net-grid)" stroke="none" />
          
          {/* Active node connections */}
          {[...Array(30)].map((_, i) => {
            const x1 = (i * 87) % 1000;
            const y1 = (i * 113) % 1000;
            const x2 = (x1 + 150) % 1000;
            const y2 = (y1 + 200) % 1000;
            return (
              <g key={i}>
                <circle cx={x1} cy={y1} r="2" fill="currentColor" opacity="0.8" stroke="none" />
                <circle cx={x2} cy={y2} r="1.5" fill="currentColor" opacity="0.6" stroke="none" />
                <path d={`M ${x1} ${y1} L ${x2} ${y2}`} strokeWidth="0.5" opacity="0.3" strokeDasharray={i % 2 === 0 ? "2 2" : "none"} />
              </g>
            );
          })}
        </g>
...
// 6. Footer Contour Map
export function FooterContourMap() {
  const [mounted, setMounted] = useState(false);
  useEffect(() => setMounted(true), []);
  ...
  return (
    <div 
      className="absolute inset-0 z-0 pointer-events-none overflow-hidden"
      style={{
        WebkitMaskImage: "linear-gradient(to bottom, transparent, white 20%, white 100%)",
        maskImage: "linear-gradient(to bottom, transparent, white 20%, white 100%)"
      }}
    >
      <svg className="absolute w-full h-[150%] top-[-25%] opacity-[0.03] dark:opacity-[0.05]" viewBox="0 0 1200 800" preserveAspectRatio="xMidYMid slice" xmlns="http://www.w3.org/2000/svg">
        <g stroke="currentColor" fill="none" strokeWidth="1" className="text-[var(--text)]">
          {[...Array(20)].map((_, i) => {
            ...
            return (
              <path
                key={i}
                d={`M -200 ${600 - yOffset} C ${300 * scale} ${750 - yOffset * 2}, ${500 * scale} ${500 - yOffset}, ${800 * scale} ${700 - yOffset * 2} S ${1000 * scale} ${400 - yOffset}, 1400 ${600 - yOffset * 2}`}
                style={{
                  transformOrigin: "center bottom",
                  transform: `scale(${1 + i * 0.01}) rotate(${i * 0.1}deg)`,
                  filter: `blur(${Math.max(0, (i - 10) * 0.5)}px)`
                }}
              />
            );
          })}
